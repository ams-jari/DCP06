...
// Unit
DCP05DistFileDlgC::DCP05DistFileDlgC(DCP05DistFileModelC* pModel):
...
DCP05DistFileDlgC::~DCP05DistFileDlgC()
{

}

void DCP05DistFileDlgC::OnInitDialog(void)
{
	...
	m_pFile->SetId(eFile);
	m_pFile->SetText(StringC(AT_DCP05,P_DCP_DISTANCE_FILE_TOK));
...
void DCP05DistFileDlgC::OnDialogActivated()
{
	if(!GetDCP05Model()->sCalcDistFile.IsEmpty())
		m_pDataModel->m_pFileFunc->setFile(GetDCP05Model()->sCalcDistFile);

	RefreshControls();
}

// Description: refresh all controls
void DCP::DCP05DistFileDlgC::RefreshControls()
{	
...
void DCP::DCP05DistFileDlgC::UpdateData()
{
	if(m_pDataModel->m_pFileFunc->IsOpen())
	{	
        GetDCP05Model()->sCalcDistFile = m_pDataModel->m_pFileFunc->getFileName();
		GetDCP05Model()->sCalcDistFile.RTrim();
	}
	else
		GetDCP05Model()->sCalcDistFile = L"";
}


// Description: only accept hello world Model objects
bool DCP::DCP05DistFileDlgC::SetModel( GUI::ModelC* pModel )
{
    // Verify type
...
DCP::DCP05ModelC* DCP::DCP05DistFileDlgC::GetDCP05Model() const
{
...
DCP::DCP05DistFileControllerC::DCP05DistFileControllerC(DCP05ModelC* pDCP05Model)
    : m_pDlg( NULL ),m_pDCP05Model(pDCP05Model)
{
    // Set title token
    // The appropriate application ID has to be set because 'C_DCP_APPLICATION_NAME_TOK'
    // is a token from the text database 'DCP05.men'
    SetTitle(StringC( AT_DCP05, T_DCP_DIST_FILE_TOK /*C_DCP_APPLICATION_NAME_TOK */));

	// FileModel
	m_pDataModel = new DCP::DCP05DistFileModelC(m_pDCP05Model); 
 
    // Create a dialog
    m_pDlg = new DCP::DCP05DistFileDlgC(m_pDataModel);  //lint !e1524 new in constructor for class 
...
DCP::DCP05DistFileControllerC::~DCP05DistFileControllerC()
{

}

// Description: Route model to everybody else
bool DCP::DCP05DistFileControllerC::SetModel( GUI::ModelC* pModel )
{
...
// OPEN
void DCP::DCP05DistFileControllerC::OnF1Pressed()
{
		if (m_pDlg == NULL)
	    {
		    USER_APP_VERIFY( false );
		...
		if(GetController(SELECT_FILE_CONTROLLER) == NULL)
		{
			...
			(void)AddController( SELECT_FILE_CONTROLLER, new DCP::DCP05SelectFileControllerC(CALCDIST_FILE, sTitle,m_pDCP05Model) );
		}
...
// NEW
void DCP::DCP05DistFileControllerC::OnF2Pressed()
{
		if (m_pDlg == NULL)
	    {
		    USER_APP_VERIFY( false );
...
// CLOSE
void DCP::DCP05DistFileControllerC::OnF5Pressed()
{	
	if(m_pDataModel->m_pFileFunc->IsOpen())
...
// CONT
void DCP::DCP05DistFileControllerC::OnF6Pressed()
{
    if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
...
// DEL
void DCP::DCP05DistFileControllerC::OnSHF2Pressed()
{
	if(!m_pDataModel->m_pFileFunc->IsOpen())
		return;
...
// VIEW
void DCP::DCP05DistFileControllerC::OnSHF5Pressed()
{
	if(!m_pDataModel->m_pFileFunc->IsOpen())
		return;
	...
	{
		(void)AddController( VIEWCDF_CONTROLLER, new DCP::DCP05ViewCdfControllerC(m_pDataModel->m_pFileFunc, m_pDlg->GetDCP05Model()) );
	}
...
void DCP::DCP05DistFileControllerC::OnActiveDialogClosed( int /*lDlgID*/, int /*lExitCode*/ )
{
}

// Description: React on close of controller
void DCP::DCP05DistFileControllerC::OnActiveControllerClosed( int lCtrlID, int lExitCode )
{
...
DCP::DCP05DistFileModelC::DCP05DistFileModelC(DCP05ModelC* pDCP05Model)
{
	m_pFileFunc = new CdfFileFunc(pDCP05Model);
	pCommon = new DCP05CommonC(pDCP05Model);

}
DCP::DCP05DistFileModelC::~DCP05DistFileModelC()
{
	if(m_pFileFunc)
	{
		delete m_pFileFunc;
...
DCP::CdfFileFunc::CdfFileFunc(DCP05ModelC* pDCP05Model): m_pFile(0), points(0), m_bExists(false),opened(0),
...
// ****************************************************************************************
DCP::CdfFileFunc::CdfFileFunc(boost::filesystem::path* FileInfo,DCP05ModelC* pDCP05Model):m_pFile(0), points(0), m_bExists(false),file_updated(0),m_pDCP05Model(pDCP05Model)
{
	m_pCommon = new DCP05CommonC(pDCP05Model);
