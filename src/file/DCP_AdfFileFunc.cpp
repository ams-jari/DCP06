...
short newpnt;
		...
		else  if(point_type == 1  && cpoint == 1)   
		{
			...
			{
				//  back,front per�kk�in
				if( (active_point_front - active_point_back) == 1)
				{
					// ***********Onko piste ensinm�inen lomakkeella
...
//int Result;
	...
	*/
	...
	{	
		...
		if(ret1 == BACK)  
		{
			if(seek_pid(id) != 0)  // Etsit��n BACK-piste
			{			
				// Jos l�ytyi ei lis�t� mit��n	
				StringC msg;
		...
		{
			// Etsit��n Ensiksi Back-piste

			strncpy(apu,id,6);apu[6] = '\0';	
			apu[strlen(id)-1] = 'B';
					
			if(seek_pid(apu) == 0) // Ei l�ytynyt, lis�t��n back
			{
			...
			{	
				if(seek_pid(id) != 0) // Etsit��n FRONT-piste				
				{
					StringC msg;
					msg.LoadTxt(AT_DCP06, M_DCP_POINT_ID_EXISTS_TOK);
					msg.Format(msg,(const wchar_t*)StringC(id));
