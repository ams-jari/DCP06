...
bool DCP::CircleFileFunc::setFile(StringC filename)
{
	...
	if(!strstr(full_path,".crl")/* || !strstr(full_path,".CRL")*/)
		strcat(full_path,".crl");
...
int  iInstNo=0;
	...
	sprintf(line_buf,"\nCircleIId:%s%c%c", point_id_buf,13,10);
	...
	sprintf(line_buf,"  x:   %9.*f%c%c",m_pModel->m_nDecimals,pModel->cx,13,10);
	fputs(line_buf,m_pFile);

	sprintf(line_buf,"  y:   %9.*f%c%c",m_pModel->m_nDecimals,pModel->cy,13,10);
	fputs(line_buf,m_pFile);

	sprintf(line_buf,"  z:   %9.*f%c%c",m_pModel->m_nDecimals,pModel->cz,13,10);
	fputs(line_buf,m_pFile);

	/*
	sprintf(line_buf,"Center point(x,y,z):%9.*f %9.*f %9.*f%c%c", m_pModel->m_nDecimals,pModel->cx, 
	...
	*/
	...
	sprintf(line_buf,"Radius:%9.*f%c%c", m_pModel->m_nDecimals,pModel->diameter,13,10); 
	fputs(line_buf,m_pFile);

	sprintf(line_buf,"Normal:%c%c",13,10);
	fputs(line_buf,m_pFile);

	sprintf(line_buf,"  i:   %9.*f%c%c",6,pModel->vi,13,10);
	fputs(line_buf,m_pFile);

	sprintf(line_buf,"  j:   %9.*f%c%c",6,pModel->vj,13,10);
	fputs(line_buf,m_pFile);

	sprintf(line_buf,"  k:   %9.*f%c%c",6,pModel->vk,13,10);
	fputs(line_buf,m_pFile);

	sprintf(line_buf,"RMS:   %9.*f%c%c", m_pModel->m_nDecimals,pModel->rms_diameter,13,10); 
	fputs(line_buf,m_pFile);
	...
	for(int i =0; i < MAX_POINTS_IN_CIRCLE; i++)
	{
		...
		{
			count++;
			sprintf(line_buf,"%-2d." DCP_POINT_ID_FMT " %+9.*f%c%c", count,pModel->circle_points[0].points[i].point_id, 
