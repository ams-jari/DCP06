...
DCP::DCP05UserDlgC::DCP05UserDlgC():GUI::ModelHandlerC(),GUI::StandardDialogC(),
...
DCP::DCP05UserDlgC::~DCP05UserDlgC()
{

}

void DCP::DCP05UserDlgC::OnInitDialog(void)
{
...
void DCP::DCP05UserDlgC::OnDialogActivated()
{
	RefreshControls();
}

void DCP::DCP05UserDlgC::UpdateData()
{
	GetDCP05Model()->m_sUser1	= m_pUser1->GetStringInputCtrl()->GetString();
	GetDCP05Model()->m_sUser2	= m_pUser2->GetStringInputCtrl()->GetString();
	GetDCP05Model()->m_sUser3	= m_pUser3->GetStringInputCtrl()->GetString();
	GetDCP05Model()->m_sUser4	= m_pUser4->GetStringInputCtrl()->GetString();
	GetDCP05Model()->m_sUser5	= m_pUser5->GetStringInputCtrl()->GetString();
	GetDCP05Model()->m_sUser6	= m_pUser6->GetStringInputCtrl()->GetString();
	GetDCP05Model()->m_sUser7	= m_pUser7->GetStringInputCtrl()->GetString();
	GetDCP05Model()->m_sUser8	= m_pUser8->GetStringInputCtrl()->GetString();
	GetDCP05Model()->m_sUser9	= m_pUser9->GetStringInputCtrl()->GetString();
	GetDCP05Model()->m_sUser10	= m_pUser10->GetStringInputCtrl()->GetString();

	// And selected user
	if(m_pUser1->HasFocus()) GetDCP05Model()->m_sUser = m_pUser1->GetStringInputCtrl()->GetString();
	else if(m_pUser2->HasFocus()) GetDCP05Model()->m_sUser = m_pUser2->GetStringInputCtrl()->GetString();
	else if(m_pUser3->HasFocus()) GetDCP05Model()->m_sUser = m_pUser3->GetStringInputCtrl()->GetString();
	else if(m_pUser4->HasFocus()) GetDCP05Model()->m_sUser = m_pUser4->GetStringInputCtrl()->GetString();
	else if(m_pUser5->HasFocus()) GetDCP05Model()->m_sUser = m_pUser5->GetStringInputCtrl()->GetString();
	else if(m_pUser6->HasFocus()) GetDCP05Model()->m_sUser = m_pUser6->GetStringInputCtrl()->GetString();
	else if(m_pUser7->HasFocus()) GetDCP05Model()->m_sUser = m_pUser7->GetStringInputCtrl()->GetString();
	else if(m_pUser8->HasFocus()) GetDCP05Model()->m_sUser = m_pUser8->GetStringInputCtrl()->GetString();
	else if(m_pUser9->HasFocus()) GetDCP05Model()->m_sUser = m_pUser9->GetStringInputCtrl()->GetString();
	else if(m_pUser10->HasFocus()) GetDCP05Model()->m_sUser = m_pUser10->GetStringInputCtrl()->GetString();
	else GetDCP05Model()->m_sUser = L"";	


}

// Description: refresh all controls
void DCP::DCP05UserDlgC::RefreshControls()
{
	if(m_pUser1 && m_pUser2 && m_pUser3 && m_pUser4 && m_pUser5 && m_pUser6 && m_pUser7 &&
		m_pUser8 && m_pUser9 && m_pUser10)
	{
		StringC sTemp= GetDCP05Model()->m_sUser1;
		sTemp.RTrim();
		m_pUser1->GetStringInputCtrl()->SetString( sTemp);
		m_pUser2->GetStringInputCtrl()->SetString( GetDCP05Model()->m_sUser2);
		m_pUser3->GetStringInputCtrl()->SetString( GetDCP05Model()->m_sUser3);
		m_pUser4->GetStringInputCtrl()->SetString( GetDCP05Model()->m_sUser4);
		m_pUser5->GetStringInputCtrl()->SetString( GetDCP05Model()->m_sUser5);
		m_pUser6->GetStringInputCtrl()->SetString( GetDCP05Model()->m_sUser6);
		m_pUser7->GetStringInputCtrl()->SetString( GetDCP05Model()->m_sUser7);
		m_pUser8->GetStringInputCtrl()->SetString( GetDCP05Model()->m_sUser8);
		m_pUser9->GetStringInputCtrl()->SetString( GetDCP05Model()->m_sUser9);
		m_pUser10->GetStringInputCtrl()->SetString( GetDCP05Model()->m_sUser10);
	}
}
// Description: only accept hello world Model objects
bool DCP::DCP05UserDlgC::SetModel( GUI::ModelC* pModel )
{
    // Verify type
...
DCP::DCP05ModelC* DCP::DCP05UserDlgC::GetDCP05Model() const
{
...
// DCP05UserControllerC
// 
DCP::DCP05UserControllerC::DCP05UserControllerC()
    : m_pDlg( NULL )
{
    // Set title token
    // The appropriate application ID has to be set because 'C_DCP_APPLICATION_NAME_TOK'
    // is a token from the text database 'DCP05.men'
    SetTitle(StringC( AT_DCP05, T_DCP_USER_TOK /*C_DCP_APPLICATION_NAME_TOK */));

    // Create a dialog
    m_pDlg = new DCP::DCP05UserDlgC;  //lint !e1524 new in constructor for class 
...
bool DCP::DCP05UserControllerC::SetModel( GUI::ModelC* pModel )
{
...
void DCP::DCP05UserControllerC::OnF1Pressed()
{
    if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
...
void DCP::DCP05UserControllerC::OnActiveDialogClosed( int /*lDlgID*/, int /*lExitCode*/ )
{
}
