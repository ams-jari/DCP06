...
#include <dcp06/orientation/DCP_DefinePlaneUserDef.hpp>
...
// Unit
//-------------------------------------------------------------------------------------------------
DCP::DCP05DomUserDefDlgC::DCP05DomUserDefDlgC(DCP::DCP05DomUserDefModelC* pDomUserDefModel):m_pPlane(0),m_pLine(0),m_pPointOffs(0),m_pPointMeas(0),
...
//-------------------------------------------------------------------------------------------------
DCP::DCP05DomUserDefDlgC::~DCP05DomUserDefDlgC()
{

}
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefDlgC::OnInitDialog(void)
{
...
void DCP::DCP05DomUserDefDlgC::load_data_from_dcp05model()
{
	m_pDataModel->domModel->old_active_coodinate_system = GetDCP05Model()->active_coodinate_system;
	memcpy(m_pDataModel->domModel->matrix,GetDCP05Model()->ocsu_matrix, sizeof(double) * 16);
	memcpy(m_pDataModel->domModel->inv_matrix,GetDCP05Model()->ocsu_inv_matrix, sizeof(double) * 16);

	m_pDataModel->domModel->dom_active_plane	= GetDCP05Model()->userdef_active_plane;
	m_pDataModel->domModel->dom_active_line	= GetDCP05Model()->userdef_active_line; 
	m_pDataModel->domModel->dom_hz_plane		=GetDCP05Model()->userdef_hz_plane;
	memcpy(&m_pDataModel->domModel->dom_plane_buff[0], &GetDCP05Model()->userdef_plane_buff[0], sizeof(S_PLANE_BUFF));
	memcpy(&m_pDataModel->domModel->dom_hz_plane_buff[0], &GetDCP05Model()->userdef_hz_plane_buff[0], sizeof(S_PLANE_BUFF));
	memcpy(&m_pDataModel->domModel->dom_line_buff[0], &GetDCP05Model()->userdef_line_buff[0], sizeof(S_LINE_BUFF));
	memcpy(&m_pDataModel->domModel->dom_ovalues_buff, &GetDCP05Model()->userdef_ovalues_buff, sizeof(S_POINT_BUFF));
	memcpy(&m_pDataModel->domModel->dom_ovalues_tool_buff, &GetDCP05Model()->userdef_ovalues_tool_buff, sizeof(S_POINT_BUFF));
	memcpy(&m_pDataModel->domModel->dom_ref_point_buff, &GetDCP05Model()->userdef_ref_point_buff, sizeof(S_POINT_BUFF));
	memcpy(&m_pDataModel->domModel->dom_rot_plane_buff, &GetDCP05Model()->userdef_rot_plane_buff, sizeof(S_POINT_BUFF));
	memcpy(&m_pDataModel->domModel->dom_rot_line_buff, &GetDCP05Model()->userdef_rot_line_buff, sizeof(S_POINT_BUFF));
	m_pDataModel->domModel->ocsd_defined = GetDCP05Model()->ocsu_defined;
	
	memcpy(&m_pDataModel->userdef_measured_points, &GetDCP05Model()->userdef_measured_points, sizeof(S_POINT_BUFF) * MAX_USERDEF_POINTS);
	memcpy(&m_pDataModel->userdef_plane_points_no, &GetDCP05Model()->userdef_plane_points_no, sizeof(short) * MAX_USERDEF_POINTS);
	memcpy(&m_pDataModel->userdef_line_points_no, &GetDCP05Model()->userdef_line_points_no, sizeof(short) * MAX_USERDEF_POINTS);
	m_pDataModel->userdef_point_no = GetDCP05Model()->userdef_point_no;
}

//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefDlgC::OnDialogActivated()
{
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefDlgC::RefreshControls()
{	
	if( m_pPlane && m_pLine && m_pPointOffs && m_pPointMeas &&	m_pRotPlane && m_pRotLine)
	{
		...
		else //(GetDCP05Model()->dom_active_plane == YZ_PLANE)
		{
		...
		// calculate status
		sStatus = L"-";
		if(GetDCP05Model()->ocsu_defined == true  && m_pDataModel->domModel->old_active_coodinate_system == OCSU)
			sStatus = L"+";
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefDlgC::UpdateData()
{
		memcpy(GetDCP05Model()->ocsu_matrix, m_pDataModel->domModel->matrix,sizeof(double) * 16);
		memcpy(GetDCP05Model()->ocsu_inv_matrix, m_pDataModel->domModel->inv_matrix, sizeof(double) * 16);
		GetDCP05Model()->userdef_active_plane	= m_pDataModel->domModel->dom_active_plane;
		GetDCP05Model()->userdef_active_line	= m_pDataModel->domModel->dom_active_line; 
		GetDCP05Model()->userdef_hz_plane		= m_pDataModel->domModel->dom_hz_plane;
		memcpy(&GetDCP05Model()->userdef_plane_buff[0], &m_pDataModel->domModel->dom_plane_buff[0], sizeof(S_PLANE_BUFF));
		memcpy(&GetDCP05Model()->userdef_hz_plane_buff[0], &m_pDataModel->domModel->dom_hz_plane_buff[0], sizeof(S_PLANE_BUFF));
		memcpy(&GetDCP05Model()->userdef_line_buff[0], &m_pDataModel->domModel->dom_line_buff[0], sizeof(S_LINE_BUFF));
		memcpy(&GetDCP05Model()->userdef_ovalues_buff, &m_pDataModel->domModel->dom_ovalues_buff, sizeof(S_POINT_BUFF));
		memcpy(&GetDCP05Model()->userdef_ovalues_tool_buff, &m_pDataModel->domModel->dom_ovalues_tool_buff, sizeof(S_POINT_BUFF));
		memcpy(&GetDCP05Model()->userdef_ref_point_buff, &m_pDataModel->domModel->dom_ref_point_buff, sizeof(S_POINT_BUFF));
		memcpy(&GetDCP05Model()->userdef_rot_plane_buff, &m_pDataModel->domModel->dom_rot_plane_buff, sizeof(S_POINT_BUFF));
		memcpy(&GetDCP05Model()->userdef_rot_line_buff, &m_pDataModel->domModel->dom_rot_line_buff, sizeof(S_POINT_BUFF));
		GetDCP05Model()->ocsu_defined = m_pDataModel->domModel->ocsd_defined;
		

		memcpy(&GetDCP05Model()->userdef_measured_points,&m_pDataModel->userdef_measured_points,  sizeof(S_POINT_BUFF) * MAX_USERDEF_POINTS);
		memcpy(&GetDCP05Model()->userdef_plane_points_no,&m_pDataModel->userdef_plane_points_no, sizeof(short) * MAX_USERDEF_POINTS);
		memcpy(&GetDCP05Model()->userdef_line_points_no,&m_pDataModel->userdef_line_points_no,  sizeof(short) * MAX_USERDEF_POINTS);
		GetDCP05Model()->userdef_point_no = m_pDataModel->userdef_point_no; 
		
		if(m_pDataModel->domModel->ocsd_defined == true)
			GetDCP05Model()->active_coodinate_system = OCSU;
		//else
		//	GetDCP05Model()->active_coodinate_system = DCS;

		// TODO CNF_KEY_USERDEF
		GetDCP05Model()->poConfigController->GetModel()->SetConfigKey(CNF_KEY_USERDEF);
...
//-------------------------------------------------------------------------------------------------
bool DCP::DCP05DomUserDefDlgC::SetModel( GUI::ModelC* pModel )
{
    // Verify type
...
//-------------------------------------------------------------------------------------------------
DCP::DCP05ModelC* DCP::DCP05DomUserDefDlgC::GetDCP05Model() const
{
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefDlgC::delete_dom()
{
		StringC strDomText;
...
// 
DCP::DCP05DomUserDefControllerC::DCP05DomUserDefControllerC(DCP05ModelC* pDCP05Model)
    : m_pDlg( NULL ),m_pDCP05Model(pDCP05Model)
{
	...
	// create model
	m_pDataModel = new DCP05DomUserDefModelC(m_pDCP05Model);

    // Create a dialog
    m_pDlg = new DCP::DCP05DomUserDefDlgC(m_pDataModel);  //lint !e1524 new in constructor for class 
...
DCP::DCP05DomUserDefControllerC::~DCP05DomUserDefControllerC()
{
	if(m_pDataModel)
	{
		delete m_pDataModel;
...
void DCP::DCP05DomUserDefControllerC::show_function_keys()
{
    FKDef vDef;
	vDef.poOwner = this;
...
//-------------------------------------------------------------------------------------------------
bool DCP::DCP05DomUserDefControllerC::SetModel( GUI::ModelC* pModel )
{
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnF1Pressed()
{	
    if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	DCP::DCP05DefinePlaneUserDefModelC* pModel = new DCP::DCP05DefinePlaneUserDefModelC();
	...
	{
		(void)AddController(DEFINE_PLANE_USERDEF_CONTROLLER, new DCP::DCP05DefinePlaneUserDefControllerC(m_pDlg->GetDCP05Model()) );
	}
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnF2Pressed()
{
	if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	DCP::DCP05DefineLineUserDefModelC* pModel = new DCP::DCP05DefineLineUserDefModelC();
	...
	{
		(void)AddController( DEFINE_LINE_USERDEF_CONTROLLER, new DCP::DCP05DefineLineUserDefControllerC(m_pDlg->GetDCP05Model()) );
	}
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnF3Pressed()
{	
	if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	{
		(void)AddController( OFFSV_CONTROLLER, new DCP::DCP05OffsvControllerC(m_pDlg->GetDCP05Model(),pModel->display));
	}
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnF4Pressed()
{
	if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
...
//-------------------------------------------------------------------------------------------------
// CALC
void DCP::DCP05DomUserDefControllerC::OnF5Pressed()
{	
    if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	{
		(void)AddController( CALC_LINE_CONTROLLER, new DCP::DCP05CalcLineControllerC(&m_pDataModel->domModel->dom_line_buff[0],ACTUAL, 0) );
	}

	(void)GetController( CALC_LINE_CONTROLLER )->SetModel(m_pDlg->GetDCP05Model());
	SetActiveController(CALC_LINE_CONTROLLER, true);

	/* siirrä tämä oikeaan paikkaan

	DCP05CalcDomC calc_dom(m_pDataModel->domModel);
	if(calc_dom.calc())
	{	
		m_pDlg->GetDCP05Model()->active_coodinate_system = OCSU;
		m_pDataModel->domModel->ocsd_defined = true;
		m_pDlg->GetDCP05Model()->ocsu_defined = true;
		// kopioi arvot DCP05Model:iin
		memcpy(m_pDlg->GetDCP05Model()->ocsu_matrix, m_pDataModel->domModel->matrix,sizeof(double) * 16);
		memcpy(m_pDlg->GetDCP05Model()->ocsu_inv_matrix, m_pDataModel->domModel->inv_matrix, sizeof(double) * 16);
		m_pDlg->GetDCP05Model()->userdef_active_plane	= m_pDataModel->domModel->dom_active_plane;
		m_pDlg->GetDCP05Model()->userdef_active_line	= m_pDataModel->domModel->dom_active_line; 
		m_pDlg->GetDCP05Model()->userdef_hz_plane		= m_pDataModel->domModel->dom_hz_plane;
		memcpy(&m_pDlg->GetDCP05Model()->userdef_plane_buff[0], &m_pDataModel->domModel->dom_plane_buff[0], sizeof(S_PLANE_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->userdef_hz_plane_buff[0], &m_pDataModel->domModel->dom_hz_plane_buff[0], sizeof(S_PLANE_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->userdef_line_buff[0], &m_pDataModel->domModel->dom_line_buff[0], sizeof(S_LINE_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->userdef_ovalues_buff, &m_pDataModel->domModel->dom_ovalues_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->userdef_ovalues_tool_buff, &m_pDataModel->domModel->dom_ovalues_tool_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->userdef_ref_point_buff, &m_pDataModel->domModel->dom_ref_point_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->userdef_rot_plane_buff, &m_pDataModel->domModel->dom_rot_plane_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->userdef_rot_line_buff, &m_pDataModel->domModel->dom_rot_line_buff, sizeof(S_POINT_BUFF));
		Close(EC_KEY_CONT);
	}
	*/
}
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnF6Pressed()
{
    if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnSHF2Pressed()
{	
	m_pDlg->delete_dom();
}
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnSHF3Pressed()
{
	if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	DCP::DCP05RotatePlaneModelC* pModel = new DCP::DCP05RotatePlaneModelC();
	...
	{
		(void)AddController( ROTATE_PLANE_CONTROLLER, new DCP::DCP05RotatePlaneControllerC(m_pDlg->GetDCP05Model()));
	}
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnSHF4Pressed()
{
	...
	DCP::DCP05RotateLineModelC* pModel = new DCP::DCP05RotateLineModelC();
	...
	{
		(void)AddController( ROTATE_LINE_CONTROLLER, new DCP::DCP05RotateLineControllerC(m_pDlg->GetDCP05Model()));
	}
...
void DCP::DCP05DomUserDefControllerC::open_meas_display() 
{
		if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnSHF5Pressed()
{
	open_meas_display();
	/*
	if (m_pDlg == NULL)
	...
	memset(&pModel->point_table[0],0,sizeof(S_POINT_BUFF) * MAX_POM_POINTS);
	memcpy(&pModel->point_table[0], &m_pDlg->GetDCP05Model()->userdef_measured_points[0], sizeof(S_POINT_BUFF) * MAX_POM_POINTS);

	//memcpy(&pModel->point_table[0], &m_pDlg->GetDataModel()->plane_buff[0].points[0], sizeof(S_POINT_BUFF) * MAX_POM_POINTS);
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnActiveDialogClosed( int lDlgID, int lExitCode )
{
	if(lDlgID == DOM_USERDEF_STATUS_DLG)
	{
		show_function_keys();
		/*
		HideFunctionKey(FK2);
...
// ================================================================================================
void DCP::DCP05DomUserDefControllerC::OnControllerActivated(void)
{
	ResetFunctionKeys();
	/*
	HideFunctionKey(FK2);
	...
	*/
	...
	DCP05UserDefStatusDlgC* poStatusDlg = new DCP05UserDefStatusDlgC(m_pDlg->GetDCP05Model(), defined);
	(void) AddDialog(DOM_USERDEF_STATUS_DLG, poStatusDlg); 	
	SetActiveDialog(DOM_USERDEF_STATUS_DLG, true);

	
}
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomUserDefControllerC::OnActiveControllerClosed( int lCtrlID, int lExitCode )
{
	...
	if(lCtrlID == MEAS_CONTROLLER)
	{
		if(lExitCode == EC_KEY_CONT)
		{
			...
			m_pDataModel->build_select_point_list();
			m_pDataModel->domModel->old_active_coodinate_system = DCS;
			m_pDlg->GetDCP05Model()->ocsu_defined = false;
		}
	...
	if(lCtrlID == DEFINE_PLANE_USERDEF_CONTROLLER && lExitCode == EC_KEY_CONT)
	{
		...
		DCP::DCP05DefinePlaneUserDefModelC* pModel = (DCP::DCP05DefinePlaneUserDefModelC*) GetController( DEFINE_PLANE_USERDEF_CONTROLLER )->GetModel();		
	...
	{
		DCP::DCP05DefineLineUserDefModelC* pModel = (DCP::DCP05DefineLineUserDefModelC*) GetController( DEFINE_LINE_USERDEF_CONTROLLER )->GetModel();		
		// copy values into dommodel
	...
	{
		DCP::DCP05OffsVModelC* pModel = (DCP::DCP05OffsVModelC*) GetController( MEASV_CONTROLLER )->GetModel();		
		// copy values into dommodel
		memcpy(&m_pDataModel->domModel->dom_ovalues_buff, &pModel->ovalues_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDataModel->domModel->dom_ovalues_tool_buff,&pModel->ovalues_tool_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDataModel->domModel->dom_ref_point_buff, &pModel->ref_point_buff, sizeof(S_POINT_BUFF));
	...
	*/
	...
	{
		DCP::DCP05RotatePlaneModelC* pModel = (DCP::DCP05RotatePlaneModelC*) GetController( ROTATE_PLANE_CONTROLLER )->GetModel();		
		// copy values into dommodel
	...
	{
		DCP::DCP05RotatePlaneModelC* pModel = (DCP::DCP05RotatePlaneModelC*) GetController( ROTATE_LINE_CONTROLLER )->GetModel();		
		// copy values into dommodel
	...
	if(lCtrlID == CALC_LINE_CONTROLLER && lExitCode == EC_KEY_CONT)
	{
		...
		if(!m_pDataModel->domModel->dom_hz_plane)
		{
			...
			{
				(void)AddController( CALC_PLANE_CONTROLLER, new DCP::DCP05CalcPlaneControllerC(&m_pDataModel->domModel->dom_plane_buff[0],ACTUAL, 0) );
			}
		...
		else
		{
			...
			{	
				...
				memcpy(&m_pDlg->GetDCP05Model()->userdef_measured_points,&m_pDataModel->userdef_measured_points,  sizeof(S_POINT_BUFF) * MAX_USERDEF_POINTS);
				memcpy(&m_pDlg->GetDCP05Model()->userdef_plane_points_no,&m_pDataModel->userdef_plane_points_no, sizeof(short) * MAX_USERDEF_POINTS);
				memcpy(&m_pDlg->GetDCP05Model()->userdef_line_points_no,&m_pDataModel->userdef_line_points_no,  sizeof(short) * MAX_USERDEF_POINTS);
				m_pDlg->GetDCP05Model()->userdef_point_no = m_pDataModel->userdef_point_no; 
...
DCP::DCP05DomUserDefModelC::DCP05DomUserDefModelC(DCP05ModelC* pDCP05Model): m_pDCP05Model(pDCP05Model)
{
...
DCP::DCP05DomUserDefModelC::~DCP05DomUserDefModelC()
{
	if(domModel != 0)
	{
		delete domModel;
...
short DCP::DCP05DomUserDefModelC::get_measured_points_count()
{
int i,count;
...
short DCP::DCP05DomUserDefModelC::get_last_defined_point()
{
	short last = 0;
	DCP05CommonC common(m_pDCP05Model);
...
bool DCP::DCP05DomUserDefModelC::is_point_id_defined()
{
	char pid[10];
	bool ret = false;
	DCP05CommonC common(m_pDCP05Model);
...
short DCP::DCP05DomUserDefModelC::build_select_point_list()
{
	memset(&select_point_list[0],0,sizeof(S_SELECT_POINTS) * MAX_USERDEF_POINTS);
...
short DCP::DCP05DomUserDefModelC::copy_measured_points()
{
	short point_no = 0;
	int i = 0;
