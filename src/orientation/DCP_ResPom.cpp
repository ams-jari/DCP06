...
// Unit
DCP::DCP05ResPomDlgC::DCP05ResPomDlgC(DCP::DCP05PomModelC *pModel):poMultiColCtrl(NULL),m_pInfo1(NULL),
	...
	m_pDataModel(pModel)
{
	...
	// load title
	sTitle.LoadTxt(AT_DCP05,T_DCP_RES_OF_POM_TOK);
	SetTitle(sTitle);
	
	
}
// Description: Destructor
DCP::DCP05ResPomDlgC::~DCP05ResPomDlgC()
{
	 if(m_pCommon)
	 {
		delete m_pCommon;
...
void DCP::DCP05ResPomDlgC::OnInitDialog(void)
{
	...
	*/
	...
	USER_APP_VERIFY(poMultiColCtrl->AddCol(StringC(AT_DCP05,P_DCP_X_TOK),CI_X));
	USER_APP_VERIFY(poMultiColCtrl->AddCol(StringC(AT_DCP05,P_DCP_Y_TOK),CI_Y));
	USER_APP_VERIFY(poMultiColCtrl->AddCol(StringC(AT_DCP05,P_DCP_Z_TOK),CI_Z));
...
void DCP::DCP05ResPomDlgC::OnF1Pressed(void)
{
	...
	{
		iMin = (!m_pCommon->get_rotation_status() && GetDCP05Model()->dom_hz_plane) ? 2 : 3;
	}
	...
	{
		if(m_pCommon->get_OCS_SCS_points_count(&m_pDataModel->point_OCS[0],&m_pDataModel->point_DCS[0], MAX_POM_POINTS) <= iMin)
		{
			StringC strText;
	...
	*/
	...
	DCP05CalcPomC calcpom(m_pDataModel,GetDCP05Model());
...
void DCP::DCP05ResPomDlgC::OnDialogActivated()
{
	RefreshControls();
} 
...
//
//*********************************************************************************
short DCP::DCP05ResPomDlgC::get_max_res()
{
...
void DCP::DCP05ResPomDlgC::RefreshControls()
{
	...
	if(!poMultiColCtrl)
		return;
	iMax = m_pCommon->get_last_defined_point(&m_pDataModel->point_OCS[0],MAX_POM_POINTS);
	iPno = get_max_res();
	// 
	//BeginDraw(); 
	char temp[20];
	...
	for(int i=0; i < iMax; i++)
	{
		...
		{
			sprintf(temp,"%+.*f", GetDCP05Model()->m_nDecimals, m_pDataModel->point_RES[i].x);
			sX = temp;

			sprintf(temp,"%+.*f", GetDCP05Model()->m_nDecimals, m_pDataModel->point_RES[i].y);
			sY = temp;

			sprintf(temp,"%+.*f", GetDCP05Model()->m_nDecimals, m_pDataModel->point_RES[i].z);
			sZ = temp;
...
*************************************************************************/
/*
double DCP::DCP05ResPomDlgC::get_max_dist_and_rms_line(S_LINE_BUFF *line, short *pno, double *rms)
{
...
*************************************************************************/
/*
double DCP::DCP05ResPomDlgC::calc_pdist(S_LINE_BUFF *line, short pno)
{
struct ams_vector m;
struct line wline;
double dist;
...
*/
void DCP::DCP05ResPomDlgC::UpdateData()
{
	/*
	StringC sSelected;
...
bool DCP::DCP05ResPomDlgC::SetModel( GUI::ModelC* pModel )
{
	  // Verify type
...
DCP::DCP05ModelC* DCP::DCP05ResPomDlgC::GetDCP05Model() const
{
...
DCP::DCP05ResPomControllerC::DCP05ResPomControllerC(DCP::DCP05PomModelC *pModel)
    : m_pDlg( NULL ),m_pDataModel(pModel)
{
    // Set title token
    // The appropriate application ID has to be set because 'C_DCP_APPLICATION_NAME_TOK'
    // is a token from the text database 'DCP05.men'
    //SetTitleTok( AT_DCP05, T_DCP_3D_FILE_TOK /*C_DCP_APPLICATION_NAME_TOK */);
	StringC sTitle;
    ...
    // Create a dialog
     m_pDlg = new DCP::DCP05ResPomDlgC(m_pDataModel);  //lint !e1524 new in constructor for class 
    (void)AddDialog( RES_POM_DLG, m_pDlg, true );
...
DCP::DCP05ResPomControllerC::~DCP05ResPomControllerC()
{

}

// Description: Route model to everybody else
bool DCP::DCP05ResPomControllerC::SetModel( GUI::ModelC* pModel )
{
...
// CONT
void DCP::DCP05ResPomControllerC::OnF6Pressed() 
{
    if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
...
void DCP::DCP05ResPomControllerC::OnActiveDialogClosed( int /*lDlgID*/, int /*lExitCode*/ )
{
}

// Description: React on close of controller
void DCP::DCP05ResPomControllerC::OnActiveControllerClosed( int lCtrlID, int lExitCode )
{
