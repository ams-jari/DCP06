...
// Unit
//-------------------------------------------------------------------------------------------------
DCP::DCP05DomDlgC::DCP05DomDlgC(DCP::DCP05DomModelC* pDomModel):m_pPlane(0),m_pLine(0),m_pPointOffs(0),m_pPointMeas(0),
...
//-------------------------------------------------------------------------------------------------
DCP::DCP05DomDlgC::~DCP05DomDlgC()
{

}
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomDlgC::OnInitDialog(void)
{
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomDlgC::OnDialogActivated()
{
	m_pDataModel->old_active_coodinate_system = GetDCP05Model()->active_coodinate_system;
	memcpy(m_pDataModel->matrix,GetDCP05Model()->ocsd_matrix, sizeof(double) * 16);
	memcpy(m_pDataModel->inv_matrix,GetDCP05Model()->ocsd_inv_matrix, sizeof(double) * 16);

	m_pDataModel->dom_active_plane	= GetDCP05Model()->dom_active_plane;
	m_pDataModel->dom_active_line	= GetDCP05Model()->dom_active_line; 
	m_pDataModel->dom_hz_plane		=GetDCP05Model()->dom_hz_plane;
	memcpy(&m_pDataModel->dom_plane_buff[0], &GetDCP05Model()->dom_plane_buff[0], sizeof(S_PLANE_BUFF));
	memcpy(&m_pDataModel->dom_hz_plane_buff[0], &GetDCP05Model()->dom_hz_plane_buff[0], sizeof(S_PLANE_BUFF));
	memcpy(&m_pDataModel->dom_line_buff[0], &GetDCP05Model()->dom_line_buff[0], sizeof(S_LINE_BUFF));
	memcpy(&m_pDataModel->dom_ovalues_buff, &GetDCP05Model()->dom_ovalues_buff, sizeof(S_POINT_BUFF));
	memcpy(&m_pDataModel->dom_ovalues_tool_buff, &GetDCP05Model()->dom_ovalues_tool_buff, sizeof(S_POINT_BUFF));
	memcpy(&m_pDataModel->dom_ref_point_buff, &GetDCP05Model()->dom_ref_point_buff, sizeof(S_POINT_BUFF));
	memcpy(&m_pDataModel->dom_rot_plane_buff, &GetDCP05Model()->dom_rot_plane_buff, sizeof(S_POINT_BUFF));
	memcpy(&m_pDataModel->dom_rot_line_buff, &GetDCP05Model()->dom_rot_line_buff, sizeof(S_POINT_BUFF));
	m_pDataModel->ocsd_defined = GetDCP05Model()->ocsd_defined;
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomDlgC::RefreshControls()
{	
	if( m_pPlane && m_pLine && m_pPointOffs && m_pPointMeas &&	m_pRotPlane && m_pRotLine)
	{
		...
		// calculate status
		sStatus = L"-";
		if(GetDCP05Model()->ocsd_defined == true  && m_pDataModel->old_active_coodinate_system == OCSD)
			sStatus = L"+";
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomDlgC::UpdateData()
{
		memcpy(GetDCP05Model()->ocsd_matrix, m_pDataModel->matrix,sizeof(double) * 16);
		memcpy(GetDCP05Model()->ocsd_inv_matrix, m_pDataModel->inv_matrix, sizeof(double) * 16);
		GetDCP05Model()->dom_active_plane	= m_pDataModel->dom_active_plane;
		GetDCP05Model()->dom_active_line	= m_pDataModel->dom_active_line; 
		GetDCP05Model()->dom_hz_plane		= m_pDataModel->dom_hz_plane;
		memcpy(&GetDCP05Model()->dom_plane_buff[0], &m_pDataModel->dom_plane_buff[0], sizeof(S_PLANE_BUFF));
		memcpy(&GetDCP05Model()->dom_hz_plane_buff[0], &m_pDataModel->dom_hz_plane_buff[0], sizeof(S_PLANE_BUFF));
		memcpy(&GetDCP05Model()->dom_line_buff[0], &m_pDataModel->dom_line_buff[0], sizeof(S_LINE_BUFF));
		memcpy(&GetDCP05Model()->dom_ovalues_buff, &m_pDataModel->dom_ovalues_buff, sizeof(S_POINT_BUFF));
		memcpy(&GetDCP05Model()->dom_ovalues_tool_buff, &m_pDataModel->dom_ovalues_tool_buff, sizeof(S_POINT_BUFF));
		memcpy(&GetDCP05Model()->dom_ref_point_buff, &m_pDataModel->dom_ref_point_buff, sizeof(S_POINT_BUFF));
		memcpy(&GetDCP05Model()->dom_rot_plane_buff, &m_pDataModel->dom_rot_plane_buff, sizeof(S_POINT_BUFF));
		memcpy(&GetDCP05Model()->dom_rot_line_buff, &m_pDataModel->dom_rot_line_buff, sizeof(S_POINT_BUFF));
		GetDCP05Model()->ocsd_defined = m_pDataModel->ocsd_defined;
		if(m_pDataModel->ocsd_defined == true)
			GetDCP05Model()->active_coodinate_system = OCSD;
		//else
		//	GetDCP05Model()->active_coodinate_system = DCS;
		GetDCP05Model()->poConfigController->GetModel()->SetConfigKey(CNF_KEY_DOM);
...
//-------------------------------------------------------------------------------------------------
bool DCP::DCP05DomDlgC::SetModel( GUI::ModelC* pModel )
{
    // Verify type
...
//-------------------------------------------------------------------------------------------------
DCP::DCP05ModelC* DCP::DCP05DomDlgC::GetDCP05Model() const
{
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomDlgC::delete_dom()
{
		StringC strDomText;
		strDomText.LoadTxt(AT_DCP05,L_DCP_DOM_TEXT_TOK);
		StringC strMsg;
		...
		if(msgbox.ShowMessageYesNo(strMsg))
		{
			...
			// added 201211
			GetDCP05Model()->ocsd_defined = false;
			GetDCP05Model()->active_coodinate_system = DCS;
...
// 
DCP::DCP05DomControllerC::DCP05DomControllerC()
    : m_pDlg( NULL )
{
	// Set title token
    // The appropriate application ID has to be set because 'C_DCP_APPLICATION_NAME_TOK'
    // is a token from the text database 'DCP05.men'
    SetTitle(StringC( AT_DCP05, T_DCP_DOM_TOK /*C_DCP_APPLICATION_NAME_TOK */));

	// create model
	m_pDataModel = new DCP05DomModelC;

    // Create a dialog
    m_pDlg = new DCP::DCP05DomDlgC(m_pDataModel);  //lint !e1524 new in constructor for class 
...
//-------------------------------------------------------------------------------------------------
bool DCP::DCP05DomControllerC::SetModel( GUI::ModelC* pModel )
{
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnF1Pressed()
{	
    if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	{
		(void)AddController(DEFINE_PLANE_CONTROLLER, new DCP::DCP05DefinePlaneControllerC(m_pDlg->GetDCP05Model()) );
	}
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnF2Pressed()
{
	if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	DCP::DCP05DefineLineModelC* pModel = new DCP::DCP05DefineLineModelC();
	pModel->active_plane	= m_pDataModel->dom_active_plane;
	pModel->active_line		= m_pDataModel->dom_active_line;
	//pModel->hz_plane		= m_pDataModel->dom_hz_plane;
	...
	{
		(void)AddController( DEFINE_LINE_CONTROLLER, new DCP::DCP05DefineLineControllerC(m_pDlg->GetDCP05Model()) );
	}
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnF3Pressed()
{	
	if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnF4Pressed()
{
	if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	{
		(void)AddController( MEASV_CONTROLLER, new DCP::DCP05MeasVControllerC(m_pDlg->GetDCP05Model()) );
	}
...
//-------------------------------------------------------------------------------------------------
// CALC
void DCP::DCP05DomControllerC::OnF5Pressed()
{	
    if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	DCP05CalcDomC calc_dom(m_pDataModel);
	if(calc_dom.calc())
	{	
		m_pDlg->GetDCP05Model()->active_coodinate_system = OCSD;
		m_pDataModel->ocsd_defined = true;
		m_pDlg->GetDCP05Model()->ocsd_defined = true;
		// kopioi arvot DCP05Model:iin
		memcpy(m_pDlg->GetDCP05Model()->ocsd_matrix, m_pDataModel->matrix,sizeof(double) * 16);
		memcpy(m_pDlg->GetDCP05Model()->ocsd_inv_matrix, m_pDataModel->inv_matrix, sizeof(double) * 16);
		m_pDlg->GetDCP05Model()->dom_active_plane	= m_pDataModel->dom_active_plane;
		m_pDlg->GetDCP05Model()->dom_active_line	= m_pDataModel->dom_active_line; 
		m_pDlg->GetDCP05Model()->dom_hz_plane		= m_pDataModel->dom_hz_plane;
		memcpy(&m_pDlg->GetDCP05Model()->dom_plane_buff[0], &m_pDataModel->dom_plane_buff[0], sizeof(S_PLANE_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->dom_hz_plane_buff[0], &m_pDataModel->dom_hz_plane_buff[0], sizeof(S_PLANE_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->dom_line_buff[0], &m_pDataModel->dom_line_buff[0], sizeof(S_LINE_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->dom_ovalues_buff, &m_pDataModel->dom_ovalues_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->dom_ovalues_tool_buff, &m_pDataModel->dom_ovalues_tool_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->dom_ref_point_buff, &m_pDataModel->dom_ref_point_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->dom_rot_plane_buff, &m_pDataModel->dom_rot_plane_buff, sizeof(S_POINT_BUFF));
		memcpy(&m_pDlg->GetDCP05Model()->dom_rot_line_buff, &m_pDataModel->dom_rot_line_buff, sizeof(S_POINT_BUFF));
		Close(EC_KEY_CONT);
	}
}

//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnF6Pressed()
{
    if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnSHF2Pressed()
{	
	m_pDlg->delete_dom();
}
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnSHF3Pressed()
{
	if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	pModel->display = DOM_DLG;
	StringC sTitle;
	sTitle.LoadTxt(AT_DCP05,T_DCP_DOM_ROTATE_PLANE_TOK);
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnSHF4Pressed()
{
	if (m_pDlg == NULL)
    {
        USER_APP_VERIFY( false );
	...
	pModel->display = DOM_DLG;
	StringC sTitle;
	sTitle.LoadTxt(AT_DCP05,T_DCP_DOM_ROTATE_LINE_TOK);
	pModel->sTitle = sTitle;
...
//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnActiveDialogClosed( int /*lDlgID*/, int /*lExitCode*/ )
{
}


//-------------------------------------------------------------------------------------------------
void DCP::DCP05DomControllerC::OnActiveControllerClosed( int lCtrlID, int lExitCode )
{
	if(lCtrlID == DEFINE_PLANE_CONTROLLER && lExitCode == EC_KEY_CONT)
	{
		
		DCP::DCP05DefinePlaneModelC* pModel = (DCP::DCP05DefinePlaneModelC*) GetController( DEFINE_PLANE_CONTROLLER )->GetModel();		
	...
	{
		DCP::DCP05DefineLineModelC* pModel = (DCP::DCP05DefineLineModelC*) GetController( DEFINE_LINE_CONTROLLER )->GetModel();		
		// copy values into dommodel
		m_pDataModel->dom_active_line	= pModel->active_line; 
		memcpy(&m_pDataModel->dom_line_buff[0], &pModel->line_buff[0], sizeof(S_LINE_BUFF));

		m_pDataModel->old_active_coodinate_system = DCS;
		m_pDlg->GetDCP05Model()->ocsd_defined = false;
...
DCP::DCP05DomModelC::DCP05DomModelC()
{
...
/*
// Instantiate template classes
DCP::DCP05DomModelC::DCP05DomModelC(DCP05DomModelC *pModel)
{
